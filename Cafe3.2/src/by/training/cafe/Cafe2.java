...
public class Cafe2 {
    ...
    public static Food getFood(Element element){
        ...
        for(int i=0; i<nodeList1.size(); i++){
            Pair<String, Integer> pair = getTypeCostPair((Element)nodeList1.get(i));
            food.getPrice().put(pair.getKey(), pair.getValue());
        }
        return food;
    }
    
    public static Pair<String, Integer> getTypeCostPair(Element element){
    ...
    public static void showMenu(ArrayList<Food> menu, String foodType){
        ...
        for(Food item:menu){
            System.out.println("1. Фото "+item.getPhoto()+"\n2. Название "+item.getName()+"\n3. Описание "+
                    item.getDescription()+"\n3. Кухня "+item.getKitchen()+"\n4. Порции: "+getStringPortions(item.getPortions())+getStringPrice(item.getPrice()));
        }
    }
    
    public static String getStringPortions(List<Integer> portions){
        String result="";
    ...
    public static String getStringPrice(Map<String, Integer> price){
        String result = "Цены: \n";
